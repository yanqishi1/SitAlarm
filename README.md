...
- 未完成校准时，系统会提示先做校准并暂停正式检测。
- 若未安装 `opencv-python`，无法进行抓拍与图片保存。
- 关闭主窗口会最小化到托盘，需通过托盘菜单退出程序。
...
```bash
python tools/head_ratio_live_debug.py
```

说明：
- 脚本会优先读取以下参考图自动计算阈值（取两者中点）：
  - 正确坐姿：`/Users/qinzhonghan/Pictures/SitAlarm/2026-02-08/115814.jpg`
  - 错误坐姿：`/Users/qinzhonghan/Pictures/SitAlarm/2026-02-08/debug_171547.jpg`
- 若自动标定失败，会回退到 `--fallback-threshold`（默认 `0.15`）。
- 运行时按键：`q` 退出，`[` / `]` 调整阈值，`r` 重新标定阈值。
